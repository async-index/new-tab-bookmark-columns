...
### Fixed
- **Turning sync on can no longer overwrite your saved layout.** On a freshly-opened tab the account's layout may not have downloaded yet; enabling sync previously misread that as "nothing saved" and pushed this device's default over it. Enabling now performs no cloud writes at all — it loads your account's layout if there is one, and otherwise just starts syncing, uploading this device's layout the next time you make an edit.
